...
if(should_load_csv){
  ...
  anime_list <- read.csv("../dataset/AnimeList.csv")
  user_list <- read.csv("../dataset/UserList.csv")
  ual_cols <- c(NA, NA, "NULL", "NULL", "NULL", "NULL", NA, "NULL", "NULL", "NULL", "NULL")
  time <- system.time(user_anime_list <- read.csv("../dataset/animelists_cleaned.csv", colClasses = ual_cols))
  print(time)
}

if(should_save_rdat){
  print("Saving Lists to RDA")
  save(user_anime_list, file="../dataset/user_anime_list.Rda")
}

if(should_load_rdat){
  print("Loading Lists from RDA")
  time <- system.time(load("../dataset/user_anime_list.Rda"))
  print(time)
}
